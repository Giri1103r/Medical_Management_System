...
const edit = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      fetch(`http://localhost:5000/api/location/selectone/${id}`)
        .then((res) => {
          if (!res.ok) {
        ...
        .then((data) => {
          setValue("name", data.location_name);
          reset(data);
        })
  ...
  const onSubmit = async (data) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/location/update/${id}`, data, {
        headers: { 'Content-Type': 'application/json' }
      });

      console.log("Updated:", res.data);
        navigate("/location", { state: { message: "Location Updated successfully!" } });
    ...
    <AdminLayout title="Edit Location">
      <div className="rounded-2xl overflow-hidden shadow-lg bg-white border">
        <div className="p-2 flex items-center justify-between">
          <h2 className="text-sm font-bold text-gray-800">Edit Location</h2>
          <button
