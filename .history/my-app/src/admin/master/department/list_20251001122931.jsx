...
const list = () => {
    ...
    // Data table columns
    const columns = [
        { name: "Name", selector: row => row.location_name, sortable: true },
        {
            name: "Status",
        ...
        { name: "Updated At", selector: row => row.updatedAt, sortable: true },
        {
            ...
            cell: row => (
                <div className="flex space-x-2">
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/location/edit/${row._id}`)}>
                        <FaPenSquare size={20} />
                    </button>
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/location/view/${row._id}`)}>
                        <FaEye size={20} />
                    </button>
    ...
    const filteredData = data.filter(item =>
        item.location_name && item.location_name.toLowerCase().includes(filterText.toLowerCase())
    );
