...
const SpecializationList = () => {
  ...
  const handleStatusChange = async (id, currentStatus) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post(`http://localhost:5000/api/specialization/status/${id}`, { status: newStatus });
        Swal.fire({
          toast: true,
          position: "top-end",
          icon: "success",
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.delete(`http://localhost:5000/api/specialization/delete/${id}`);
        Swal.fire({
          toast: true,
          position: "top-end",
          icon: "success",
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  value={statusDropdown.find(opt => opt.value === field.value) || null} // select correct option
                  placeholder="-- Select Status --"
                  isClearable
                  onChange={(selected) => field.onChange(selected ? selected.value : null)} // store only value
