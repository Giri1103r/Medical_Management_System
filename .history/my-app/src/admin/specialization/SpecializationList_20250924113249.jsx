...
const SpecializationList = () => {
  ...
  const handleStatusChange = async (id, currentStatus) => {
    ...
    const title = currentStatus == 1 ? "Do you Want to Inactive Status" ? "Do You Want to Activate the Status";
    const text = currentStatus == 1 ? "Active" ? "In-active";
    const btnColors = currentStatus == 1 ? "#dc3545" : "#7ddc35";
 ...
 if (result.isConfirmed) {
    try {
      const response = await axios.post(`http://localhost:5000/api/specialization/status`, {
        id,
        status: newStatus
      });
