...
const SpecializationList = () => {
 ...
 const handleStatusChange = async (id, currentStatus) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.patch(`http://localhost:5000/api/specialization/status/${id}`, {
        status: newStatus
      });
      ...
      // Optimistically update the status in state without re-fetching all
      setData(prevData =>
        prevData.map(item =>
