...
const SpecializationAdd = () => {
  ...
  const checkUniquesness = async (field,value)=>{
    const rest = await res.get(`http://localhost:5000/api/specialization/uniqueness?field=${field}&value=${value}`);
    const data = await res.json();
    return data.exists ? ` ${field} Should be Unique` : true;
  }
                ...
                <div>
                  ...
                  <input
                    ...
                    {...register("name", { required: "Name is required",
                      validate:(value) => checkUniquesness("name",value)
                     })}
                  />
