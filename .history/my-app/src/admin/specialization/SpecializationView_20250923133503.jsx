  ...
  return (
    <AdminLayout title="View Specialization">
      ...
      <div className="max-w-2xl mx-auto mt-6 px-4 bg-white shadow p-6 rounded-lg">
        <p><span className="font-bold">Name:</span> {specialization.name}</p>
        <p><span className="font-bold">Created At:</span> {new Date(specialization.createdAt).toLocaleString()}</p>
        <p><span className="font-bold">Status:</span> {specialization.status === 1 ? "Active" : "Inactive"}</p>
      </div>
    </AdminLayout>
  );
}
