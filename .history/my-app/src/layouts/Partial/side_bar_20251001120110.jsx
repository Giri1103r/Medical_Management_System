...
const SideBarItem = ({ item, menuData }) => {
  ...
  const children = menuData.filter((child) => String(child.parent_id) === String(item._id));
  ...
  return hasChildren ? (
    <div className="mb-1">
      <button
        ...
        className="flex items-center w-full px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-full"
      >
    ...
    <button
      ...
      className="flex items-center w-full px-4 py-2 text-gray-700 bg-gray-200 rounded-full hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-1"
    >
      {item.icon && <i className={`${item.icon} w-5 h-5 mr-2`} />}
