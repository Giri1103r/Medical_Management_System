...
const SideBarItem = ({ item, menuData }) => {
  ...
  // Use item.color or fallback to gray
  const bgColor = item.color || "bg-gray-200";
  const hoverColor = item.color ? "" : "hover:bg-gray-300";

  return hasChildren ? (
    <div className="mb-2">
      <button
        onClick={() => setOpen(!open)}
        className={`flex items-center w-full px-4 py-2 text-black rounded-lg ${bgColor} hover:brightness-90`}
      >
        {item.icon && <i className={`${item.icon} w-5 h-5 mr-2`} />}
        <span className="flex-1">{item.name || "NO NAME"}</span>
        <span className="ml-auto text-white">{open ? "▾" : "▸"}</span>
      </button>
      {open && (
    ...
    <button
      ...
      className={`flex items-center w-full px-4 py-2 text-white rounded-lg mb-1 ${bgColor} ${hoverColor} hover:brightness-90`}
    >
      {item.icon && <i className={`${item.icon} w-5 h-5 mr-2`} />}
...
const SidebarMenu = ({ menuData }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen shadow-md p-4 w-64 transform transition-transform duration-300 ease-in-out
