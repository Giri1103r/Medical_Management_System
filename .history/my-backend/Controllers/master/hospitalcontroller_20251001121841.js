const Hospital = require('../../Models/master/location')
...
const index = async (req, res) => {
    try {
        const locations = await Hospital.find({
            trash: false
        }).sort({ _id: -1 });
...
const edit = async (req, res) => {
    try {
        ...
        const locations = await Hospital.findById(id);
        if (!locations) {
...
const updates = async (req, res) => {
    try {
        const { id } = req.params;
        const { name } = req.body;
        const locations = await Hospital.findByIdAndUpdate(id, { name }, { new: true });
        if (!loations) {
            return res.status(404).json({ message: "Hospitals not found" });
        }
        res.status(200).json({ message: "Hospitals updated successfully", locations });
    }
    catch (error) {
        res.status(500).json({ message: "Server error", error: error.message })
    }
...
const status = async (req, res) => {
    try {
        ...
        const locations = await Hospital.findByIdAndUpdate(
            id,
            { status },
            { new: true }
        );
