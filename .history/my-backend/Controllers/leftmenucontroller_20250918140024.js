...
const store = async (req, res) => {
    ...
    try {
        const { name, name_key, link, icon, parent_id, module_id, is_parent, sort_order,mod_key} = req.body;   
        const newMenu = new LeftMenu({ name, namekey:name_key, link, icon, parent_id, module_id, is_parent, sort_order,modkey:mod_key,status:1 });
