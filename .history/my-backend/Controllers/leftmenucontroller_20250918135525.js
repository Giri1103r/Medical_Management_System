...
const store = async (req, res) => {
   ...
   await body('link').trim().notEmpty().withMessage('URL is Required').run(req);
   await body('icon').trim().notEmpty().withMessage('Icon is Required').run(req);
   await body('module').trim().notEmpty().withMessage('Module is Required').run(req);
   await body('parent_id').trim().notEmpty().withMessage('Parent Menu is Required').run(req);
