...
const status = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body; // <-- you need status, not name

    // Fix syntax: { new: true } should be a separate object, not inside status
...
module.exports = { store, index, edit, updates, selectone,status }