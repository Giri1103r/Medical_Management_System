...
const list = () => {
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.post(`http://localhost:5000/api/hospital/status/${id}`, { status: newStatus });
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:5000/api/hospital/delete/${id}`);
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
        ...
        { name: "Updated At", selector: row => row.updatedAt, sortable: true },
        {
            ...
            cell: row => (
                <div className="flex space-x-2">
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/hospital/edit/${row._id}`)}>
                        <FaPenSquare size={20} />
                    </button>
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/hospital/view/${row._id}`)}>
                        <FaEye size={20} />
                    </button>
    ...
    const filteredData = data.filter(item =>
        item.hospital_name && item.hospital_name.toLowerCase().includes(filterText.toLowerCase())
    );
            ...
            >
                <div className="p-6 bg-white border rounded-2xl shadow-md mb-4 flex flex-wrap gap-6 items-center">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-2">
                        <label htmlFor="hospital" className="whitespace-nowrap">Hospital</label>
                        <input
                            type="text"
