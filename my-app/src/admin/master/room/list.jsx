...
const list = () => {
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.post(`http://localhost:5000/api/room/status/${id}`, { status: newStatus });
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:5000/api/room/delete/${id}`);
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
    ...
    const columns = [
        ...
        { name: "Department Name", selector: row => row.room_name, sortable: true },
        {
            name: "Status",
        ...
        { name: "Updated At", selector: row => row.updatedAt, sortable: true },
        {
            ...
            cell: row => (
                <div className="flex space-x-2">
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/room/edit/${row._id}`)}>
                        <FaPenSquare size={20} />
                    </button>
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/room/view/${row._id}`)}>
                        <FaEye size={20} />
                    </button>
