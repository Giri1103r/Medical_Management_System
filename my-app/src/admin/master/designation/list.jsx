...
const list = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/api/designation/list")
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.post(`http://localhost:5000/api/designation/status/${id}`, { status: newStatus });
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.delete(`http://localhost:5000/api/designation/delete/${id}`);
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
    ...
    const columns = [
        ...
        { name: "Department Name", selector: row => row.designation_name, sortable: true },
        {
            name: "Status",
        ...
        { name: "Updated At", selector: row => row.updatedAt, sortable: true },
        {
            ...
            cell: row => (
                <div className="flex space-x-2">
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/designation/edit/${row._id}`)}>
                        <FaPenSquare size={20} />
                    </button>
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/designation/view/${row._id}`)}>
                        <FaEye size={20} />
                    </button>
    ...
    const filteredData = data.filter(item =>
        item.designation_name && item.designation_name.toLowerCase().includes(filterText.toLowerCase())
    );
        ...
        <AdminLayout>
            <div className="rounded-2xl overflow-hidden shadow-lg bg-white border mb-4">
                <div className="p-2 flex items-center justify-between">
                    <h2 className="text-sm font-bold text-gray-800">Department</h2>
            ...
            >
                <div className="p-6 bg-white border rounded-2xl shadow-md mb-4 flex flex-wrap gap-6 items-center">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-2">
                        <label htmlFor="specialization" className="whitespace-nowrap">Department</label>
                        <input
                            type="text"
