...
const list = () => {
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await axios.post(`http://localhost:5000/api/department/status/${id}`, { status: newStatus });
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    icon: "success",
        ...
        { name: "Updated At", selector: row => row.updatedAt, sortable: true },
        {
            ...
            cell: row => (
                <div className="flex space-x-2">
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/department/edit/${row._id}`)}>
                        <FaPenSquare size={20} />
                    </button>
                    <button className='text-blue-500 hover:text-blue-700' onClick={() => navigate(`/department/view/${row._id}`)}>
                        <FaEye size={20} />
                    </button>
